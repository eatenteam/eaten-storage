...
func (s *Server) handleMallsGet() httprouter.Handle {
    collection := s.db.Database("storage").Collection("mall")
...
func (s *Server) handleMallsPost() httprouter.Handle {
    ...
    return func(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
        ...
        if err := decodeBody(r, &mall); err != nil {
            log.Println(mall.Brand)
            log.Println(mall.Active)
        ...
        if err != nil {
            respondErr(w, r, http.StatusBadRequest, "failed to insert document into the database")
            return
        }
